...
/*   Updated: 2023/06/05 12:04:35 by bgales           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Form::signAuthorized(Bureaucrat a){
	...
	if (a.getGrade() > getGrade("sign")){
		...
		std::cout << BR << "*" + a.getName() + " couldn't sign " + getName()
			+	" formulary*" << GREEN;
...
void	Form::execAuthorized(Bureaucrat a) const{

	if (a.getGrade() > getGrade("exec")){
		std::cout << BR << "* " << a.getName() << " could'nt execute "
			<< getName() + " formulary * " << GREEN;
		throw GradeTooLowException();
	}
	if (_status == UNSIGNED){
		std::cout << BR << "*" << a.getName() << " could'nt execute "
...
const char *Form::UnsignedException::what() const throw(){
	return ("Formulary is unsigned exception.");
}

const char *Form::AlreadySignedException::what() const throw(){
	return (" Formulary is already signed exception.");
}